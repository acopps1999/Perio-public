...
function ResearchModal({
  ...
  const renderResearchContent = () => {
    ...
    // If no specific product is selected but we have a condition
    if (!selectedResearchProduct) {
      // Option 1: Show a message asking to select a product
      return (
        <div className="py-10 text-center text-gray-500">
          <BookOpen size={48} className="mx-auto mb-4 text-gray-300" />
          <p>Please select a specific product to view its research articles.</p>
        </div>
      );
    }
